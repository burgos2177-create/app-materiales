...
9. **Materiales ad-hoc**: el almacenista puede crear materiales que no estén en el catálogo OPUS desde el modal de agregar item (botón "+ Nuevo material"). Quedan con `origen: 'ad_hoc'` y `creadoPor: uid`. El re-import del XLS los **preserva** (en `saveCatalogoMateriales`), salvo que OPUS produzca el mismo `materialKey` (misma clave + descripción + unidad) — en ese caso lo absorbe. Es el release valve para la realidad de obra que diverge del catálogo.
    **Son editables por completo** (2026-08-11) desde el catálogo — badge `ad-hoc ✎` o la celda de unidad: clave, descripción, unidad, familia, marca y proveedor. Como el `materialKey` se deriva de `clave|descripcion|unidad`, cambiar cualquiera de esos tres **re-keyea** el registro (`updateMaterialAdHoc`): escribe el item en la key nueva, migra las referencias en requisiciones/recepciones/salidas y en los snapshots vivos de `/shared/buzon` de esa obra, y hasta el final borra el viejo. Guarda `keyAnterior` para auditoría. Si la identidad destino ya la ocupa otro material, se bloquea (fusionar sería destructivo). Los materiales de **OPUS no** cambian identidad aquí: se corrigen en OPUS y se re-importan.
...
/shared/materiales/{obraId}/recepciones/{recId}:
  ...
  fondoCaja,                  # 'transferencia' | 'efectivo' — de qué fondo de caja chica
                              # salió el dinero. Se elige al CREAR la recepción (si es de
                              # caja chica) o al enviar una de OC con formaPago='caja_chica'.
